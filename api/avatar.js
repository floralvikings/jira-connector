...
function AvatarClient(jiraClient) {
  ...
  this.createTemporaryAvatar = function(opts, callback) {
    ...
    const size = fs.statSync(opts.avatarFilePath).size;
    const name = path.basename(opts.avatarFilePath);
    const options = {
      method: 'POST',
      json: true,
      followAllRedirects: true,
      uri: this.jiraClient.buildURL(
        '/avatar/' + opts.avatarType + '/temporary'
      ),
      headers: {
  ...
  this.cropTemporaryAvatar = function(opts, callback) {
    ...
    const options = {
      ...
      followAllRedirects: true,
      uri: this.jiraClient.buildURL(
        '/avatar/' + opts.avatarType + '/temporaryCrop'
      ),
      headers: {
        'X-Atlassian-Token': 'no-check'
      },
