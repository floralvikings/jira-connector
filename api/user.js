...
function UserClient(jiraClient) {
  ...
  this.multiProjectSearchAssignable = function(opts, callback) {
    let projectKeyString = '';
    if (opts.projectKeys) {
      opts.projectKeys.forEach(key => {
  ...
  this.createTemporaryAvatar = function(opts, callback) {
    let extension = path.extname(opts.filepath).slice(1);
    const baseName = path.basename(opts.filepath);
